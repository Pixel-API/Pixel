...
const testAntigravityOAuthClientID = "test-antigravity-client-id"

func TestMain(m *testing.M) {
	ClientID = testAntigravityOAuthClientID
	os.Exit(m.Run())
}
...
func TestBuildAuthorizationURL_参数验证(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("BuildAuthorizationURL 返回错误: %v", err)
	}
...
func TestConstants_值正确(t *testing.T) {
	...
	if AntigravityOAuthClientIDEnv != "ANTIGRAVITY_OAUTH_CLIENT_ID" {
		t.Errorf("AntigravityOAuthClientIDEnv 不匹配: got %s", AntigravityOAuthClientIDEnv)
	}
	if AntigravityOAuthClientSecretEnv != "ANTIGRAVITY_OAUTH_CLIENT_SECRET" {
		t.Errorf("AntigravityOAuthClientSecretEnv 不匹配: got %s", AntigravityOAuthClientSecretEnv)
	}
