...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	if effective.ClientID == "" && effective.ClientSecret == "" {
		clientID, err := geminiCLIOAuthClientID()
		if err != nil {
			return OAuthConfig{}, err
		}
		secret, err := geminiCLIOAuthClientSecret()
		if err != nil {
			return OAuthConfig{}, err
		}
	...
	isBuiltinClient := IsGeminiCLIOAuthClientID(effective.ClientID)
...
func geminiCLIOAuthClientID() (string, error) {
	...
	return "", infraerrors.Newf(http.StatusBadRequest, "GEMINI_CLI_OAUTH_CLIENT_ID_MISSING", "built-in Gemini CLI OAuth client_id is not configured; set %s or provide a custom OAuth client", GeminiCLIOAuthClientIDEnv)
}

func geminiCLIOAuthClientSecret() (string, error) {
	...
	return "", infraerrors.Newf(http.StatusBadRequest, "GEMINI_CLI_OAUTH_CLIENT_SECRET_MISSING", "built-in Gemini CLI OAuth client_secret is not configured; set %s or provide a custom OAuth client", GeminiCLIOAuthClientSecretEnv)
}

func IsGeminiCLIOAuthClientID(clientID string) bool {
	configuredClientID, err := geminiCLIOAuthClientID()
	return err == nil && strings.TrimSpace(clientID) == configuredClientID
}
