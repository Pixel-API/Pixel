...
func TestOpenAIGatewayService_SelectAccountWithScheduler_DefaultDisabledUsesRuntimeTTFTTieBreaker(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	slowTTFT := 9000
	fastTTFT := 500
	svc.ReportOpenAIAccountScheduleResult(36101, true, &slowTTFT)
	svc.ReportOpenAIAccountScheduleResult(36102, true, &fastTTFT)
	require.False(t, svc.isOpenAIAdvancedSchedulerEnabled(ctx))
