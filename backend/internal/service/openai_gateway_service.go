			...
			if err == nil {
				...
				if !clearSticky && isOpenAIAccountEligibleForRequest(account, requestedModel, false) {
					...
					if account == nil {
						...
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else if s.shouldEscapeOpenAIStickyAccountByRuntimeStats(account.ID) {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else {
	...
	if err != nil {
		...
		if len(available) > 0 {
			sort.SliceStable(available, func(i, j int) bool {
				...
				if cmp := s.compareOpenAIAccountsByRuntimeStats(a.account.ID, b.account.ID); cmp != 0 {
					return cmp < 0
				}
				switch {
...
func (s *OpenAIGatewayService) shouldEscapeOpenAIStickyAccountByRuntimeStats(accountID int64) bool {
	...
	return hasTTFT && ttft >= 8000
}

func (s *OpenAIGatewayService) compareOpenAIAccountsByRuntimeStats(leftID, rightID int64) int {
	if s == nil || s.openaiAccountStats == nil || leftID <= 0 || rightID <= 0 {
		return 0
	}
	leftErr, leftTTFT, leftHasTTFT := s.openaiAccountStats.snapshot(leftID)
	rightErr, rightTTFT, rightHasTTFT := s.openaiAccountStats.snapshot(rightID)
	if leftErr != rightErr {
		if leftErr < rightErr {
			return -1
		}
...
type openAIStreamLineReadResult struct {
	line string
	err  error
	done bool
}
...
func openAIStreamDataStartsClientOutput(data, eventType string) bool {
	...
	if openAIStreamDataStartsFirstContentToken(trimmed, eventType) {
		return true
	}
	return false
}

func openAIStreamDataStartsFirstContentToken(data, eventType string) bool {
	trimmed := strings.TrimSpace(data)
	...
	case "response.content_part.added", "response.content_part.done":
		return strings.TrimSpace(gjson.Get(trimmed, "part.text").String()) != ""
	case "response.output_item.done":
		return strings.TrimSpace(gjson.Get(trimmed, "item.content.0.text").String()) != ""
	}
	if content := strings.TrimSpace(gjson.Get(trimmed, "choices.0.delta.content").String()); content != "" {
		return true
	}
	if refusal := strings.TrimSpace(gjson.Get(trimmed, "choices.0.delta.refusal").String()); refusal != "" {
		return true
	}
	if text := strings.TrimSpace(gjson.Get(trimmed, "delta").String()); text != "" &&
		(strings.Contains(eventType, "text") || strings.Contains(eventType, "refusal")) {
		return true
	}
	return false
}

func (s *OpenAIGatewayService) openAIFirstContentTokenFailoverTimeout() time.Duration {
	const defaultTimeout = 10 * time.Second
	if s == nil || s.cfg == nil || s.cfg.Gateway.StreamDataIntervalTimeout <= 0 {
		return defaultTimeout
	}
	configured := time.Duration(s.cfg.Gateway.StreamDataIntervalTimeout) * time.Second
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	lineCh := make(chan openAIStreamLineReadResult, 1)
	go func() {
		for scanner.Scan() {
			lineCh <- openAIStreamLineReadResult{line: scanner.Text()}
		}
		lineCh <- openAIStreamLineReadResult{err: scanner.Err(), done: true}
	}()
	firstContentTimeout := s.openAIFirstContentTokenFailoverTimeout()
	...
	for {
		var readResult openAIStreamLineReadResult
		if firstTokenMs == nil && !openAIStreamClientOutputStarted(c, clientOutputStarted) {
			select {
			case readResult = <-lineCh:
			case <-firstContentTimer.C:
				msg := fmt.Sprintf("OpenAI stream first content token timeout after %s", firstContentTimeout)
			...
			case <-ctx.Done():
				return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs}, fmt.Errorf("stream usage incomplete: %w", ctx.Err())
			}
		} else {
			select {
			case readResult = <-lineCh:
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if firstTokenMs == nil && openAIStreamDataStartsFirstContentToken(trimmedData, eventType) {
