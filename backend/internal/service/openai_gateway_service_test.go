...
func TestOpenAISelectAccountWithLoadAwareness_StickySlowRuntimeClearsSession(t *testing.T) {
	sessionHash := "session-slow"
	groupID := int64(1)
	now := time.Now()
	later := now.Add(30 * time.Second)
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, LastUsedAt: &now},
			{ID: 2, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, LastUsedAt: &later},
		},
	}
	cache := &stubGatewayCache{
		sessionBindings: map[string]int64{"openai:" + sessionHash: 1},
	...
	slowTTFT := 9000
	fastTTFT := 500
	svc.ReportOpenAIAccountScheduleResult(1, true, &slowTTFT)
	svc.ReportOpenAIAccountScheduleResult(2, true, &fastTTFT)
...
func TestOpenAIStreamingPassthroughFirstTokenUsesContentDelta(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			"",
			"event: response.done",
			`data: {"type":"response.done","response":{"usage":{"input_tokens":1,"output_tokens":1}}}`,
			"",
		}, "\n"))),
	...
	result, err := svc.handleStreamingResponsePassthrough(c.Request.Context(), resp, c, &Account{ID: 1}, start, "", "")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, result.firstTokenMs)
	require.GreaterOrEqual(t, *result.firstTokenMs, 1400)
...
func TestOpenAIStreamingPassthroughFirstContentTimeoutFailsOverBeforeWriting(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
