...
# GEMINI_CLI_OAUTH_CLIENT_ID=your-gemini-cli-client-id
# GEMINI_CLI_OAUTH_CLIENT_SECRET=your-gemini-cli-client-secret
```
...
2. Add your OAuth credentials in the `[Service]` section (after the existing `Environment=` lines):
   ...
   ```ini
   Environment=GEMINI_CLI_OAUTH_CLIENT_ID=your-gemini-cli-client-id
   Environment=GEMINI_CLI_OAUTH_CLIENT_SECRET=your-gemini-cli-client-secret
   ```
...
> **Note:** Code Assist OAuth uses the Gemini CLI client flow. Set `GEMINI_CLI_OAUTH_CLIENT_ID` and `GEMINI_CLI_OAUTH_CLIENT_SECRET` in the runtime environment before enabling it.
